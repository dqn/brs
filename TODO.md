...
#### 18e-10: SkinHeader replacement + NullSongDatabaseAccessor module extraction (complete)
...
- **beatoraja-select remaining stubs:** MainState trait (empty marker), RandomCourseData/RandomStageData (blocked: need random course generation logic), RankingData/RankingDataCache (blocked: need IR integration), SkinText/SkinNumber/SkinImage/SkinObject/SkinObjectRenderer (blocked: rendering pipeline), EventType (used by music_selector execute_event), AudioDriver (used by preview_music_processor), DownloadTask/DownloadTaskState/DownloadTaskStatus (used by download_task_menu). BMSPlayerMode/PlayerInformation replaced (18e-7). Dead Pair removed (18e-7). SongManagerMenu delegates to beatoraja-types (18e-8). BMSPlayerInputProcessor/ControlKeys/KeyCommand replaced (18e-9). TimerState removed as dead code (18e-9). SkinHeader replaced with real type (18e-10). NullSongDatabaseAccessor moved to own module (18e-10)
