...
- [ ] beatoraja-result: Partially unblocked — 3 mutable getters added to `PlayerResourceAccess` trait (`get_score_data_mut`, `get_course_replay_mut`, `get_course_gauge_mut`). Still blocked on: types not on trait (`BMSModel`, `RankingData`, `BMSPlayerMode`), stub type mismatches (`FloatArray` vs `Vec<f32>`, `GdxArray` vs `Vec`, `GrooveGaugeStub` vs `GrooveGauge`), and `get_replay_data_mut`/`get_course_score_data_mut` (needed in stub but not actually called — can be removed). Next step: convert callers from stub types to real types, then swap to `Box<dyn PlayerResourceAccess>` wrapper
...
- [ ] beatoraja-result stub type conversion needed — `FloatArray` → `Vec<f32>`, `GdxArray<T>` → `Vec<T>`, `GrooveGaugeStub` → real `GrooveGauge` (from beatoraja-play via beatoraja-types). Callers in music_result.rs, course_result.rs, skin_gauge_graph_object.rs need updating. This unblocks PlayerResource → `Box<dyn PlayerResourceAccess>` migration
