...
fn decode_wav_to_i16(filename: &str) -> (Vec<i16>, u16, u32) {
    ...
    let pcm = rubato::audio::decode::load_audio(&path)
        .unwrap_or_else(|e| panic!("Failed to decode {}: {}", filename, e));
    let i16_samples = rubato::audio::bms_renderer::f32_to_i16(&pcm.samples);
...
fn resample_44100_to_22050() {
    ...
    let pcm = rubato::audio::decode::load_audio(&path).unwrap();

    let resampled = pcm.change_sample_rate(tc.target_rate.unwrap());
    let rust_samples = rubato::audio::bms_renderer::f32_to_i16(&resampled.samples);
...
fn channel_mono_to_stereo() {
    ...
    let rust_samples = rubato::audio::bms_renderer::f32_to_i16(&converted.samples);
