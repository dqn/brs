...
- **Bitmap font vertical anchor parity:** `SkinTextBitmap.draw()` receives the line top from the caller. Glyph `yoffset` must therefore be converted from BMFont's top-origin semantics without reapplying the font base offset, or list text can stay visible but shift vertically within the bar.
- **Render-capture identifier parity:** Bitmap font glyph quads in runtime capture use generated texture keys such as `__pixmap_*`, not the source `.fnt` path. Capture-based verification must filter on the renderer's actual emitted keys and the target widget bounds before concluding that text is missing.
- **Render-capture spatial assertion validation:** When a plan proposes render-capture assertions with hardcoded spatial expectations (expected center, coordinate ranges, tolerances), treat them as hypotheses, not ground truth. Run the capture first without position-specific assertions to observe actual quad distribution, then set expectations from observed data. Songlist-like renderers draw across multiple bar rows spanning a wide vertical range, making single-center assertions inappropriate.
