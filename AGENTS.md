...
| 18e-2 | Lifecycle stub replacement (partial): MainController stubs removed from 5 crates (obs: `state_type()` on MainState trait; external: `NullMainController`; decide: unused trait impl removed; select: dead code deleted, `MainState` simplified to empty marker trait; modmenu: dead code removed + stub PlayerConfig/SkinConfig/SkinConfigProperty replaced with real beatoraja-types, ~15 call sites updated for `Option<>` handling). beatoraja-ir had no stub. md-processor reclassified as intentional adapter pattern (1-method `MainControllerRef` trait). beatoraja-result: 3 unused MainController methods pruned (get_config, get_player_config, change_state); stub type conversion done (FloatArray→Vec<f32>, GdxArray→Vec, GrooveGaugeStub→real GrooveGauge from beatoraja-types); PlayerResource wrapper complete (`Box<dyn PlayerResourceAccess>` + crate-local BMSModel/BMSPlayerMode/RankingData/course_bms_models fields); callers updated for Option<> on get_songdata/get_replay_data/get_course_data, by-value add_course_replay, get_course_score_data_mut→get/clone/set. `PlayerResourceAccess` trait expanded with 3 mutable getters (`get_score_data_mut`, `get_course_replay_mut`, `get_course_gauge_mut`). `NullPlayerResource` changed from unit struct to `#[derive(Default)]` with fields. beatoraja-core `PlayerResource` impl updated. PlayerResource wrapper migration complete for all 6 crates (select: deleted; decide: trait-based; external: wrapper + Mode; result: wrapper + BMSModel/BMSPlayerMode/RankingData). Dead `src/pending/` files deleted from golden-master. 1241 tests pass |
...
- ~~**PlayerResource (select/decide/external/result):**~~ → resolved: select empty struct deleted, decide replaced with `Box<dyn PlayerResourceAccess>`, external replaced with `Box<dyn PlayerResourceAccess>` wrapper + `original_mode: Mode`, result replaced with `Box<dyn PlayerResourceAccess>` wrapper + crate-local fields (BMSModel, BMSPlayerMode, RankingData, course_bms_models) (Phase 18e-2)
- **Lifecycle:** MainController stubs: result (6 methods actively used, 3 unused pruned, blocked on type mismatches), md-processor (intentional adapter pattern, deferred). modmenu retains 3-method MainController stub (get_config, get_player_config, save_config) until real MainController exists. **PlayerResource wrapper migration complete for all crates.**
...
- **PlayerResource stub→trait migration pattern:** For crates with only trait-compatible methods (decide: `set_org_gauge_option`, `get_player_config().gauge`), direct swap to `Box<dyn PlayerResourceAccess>` works cleanly. For crates with direct getters returning non-optional concrete types (external: `get_songdata() -> &SongData` vs trait `Option<&SongData>`), callers must be updated to handle `Option<>`. For crate-local methods not on trait (`get_original_mode()` returning `Mode` from bms-model), use a wrapper struct holding `Box<dyn PlayerResourceAccess>` + the extra field, with delegating methods. No extension trait needed when the extra method is only used within one crate. For crates with many non-trait types (result: BMSModel, BMSPlayerMode, RankingData, course_bms_models), the wrapper struct pattern scales to multiple extra fields. Methods returning `&mut T` not on trait (e.g., `get_course_score_data_mut`) can be converted to get/clone/set pattern via `get_course_score_data().cloned()` + `set_course_score_data()`. Methods only existing on stub but never called (`get_replay_data_mut`, `reload_bms_file`) should be deleted rather than ported to wrapper.
