...
### 入力
- BMS 7-key のみ有効（PMS 9-key / DP 14-key は一時無効化）
- キーボード + ゲームパッド入力を統合判定
...
- 判定ラインにボムエフェクト(拡散リング)を追加し、色/サイズ/時間は `effects.bomb` で調整可能
- IIDX レイアウトのスキンでプレイ/グラフ/ヘッダ/右下パネルの配置を調整可能（`layout.iidx` 系）

### BGA
- 画像と動画をロード (動画は ffmpeg)
- Base/Poor/Overlay の 3 レイヤー
- 同名の動画ファイルが複数ある場合は解像度が高いものを優先して選択 (例: `sample/bms` の bga.wmv 512x512 より bga.mp4 1280x720 を優先)
- 再生位置に合わせて動画フレームを更新
