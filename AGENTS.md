...
| 18e-10 | SkinHeader stub → real `pub use beatoraja_skin::skin_header::SkinHeader` in beatoraja-select. NullSongDatabaseAccessor moved from stubs.rs to own module. **All actionable stub replacements exhausted** |
...
- **Remaining stubs.rs (audited — all actionable replacements exhausted):** 16 files, ~2,650 lines total. beatoraja-result: MainController (6 methods), SkinObjectData. beatoraja-select: MainState, RandomCourseData/RankingData (blocked: IR), SkinText/SkinNumber/SkinImage/SkinObject/SkinObjectRenderer (blocked: rendering), EventType/AudioDriver (in use), DownloadTask (in use). SkinHeader replaced (18e-10), NullSongDatabaseAccessor moved to own module (18e-10). beatoraja-modmenu: Skin/SkinObject/Rectangle, MainState, MainController, MusicSelector/Bar/SongBar. beatoraja-decide: MainControllerRef + InputProcessor/AudioProcessor stubs. beatoraja-external: LibGDX stubs (Pixmap/GdxGraphics/BufferUtils/PixmapIO, used by screenshot exporters), Twitter4j (intentional bail), Property stubs. **beatoraja-pattern: zero stubs (18e-8).** All remaining stubs blocked by: rendering pipeline, IR/network layer, database queries, or per-screen MainState implementations
