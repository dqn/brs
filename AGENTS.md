...
- **PlayerResourceAccess trait gap (partially resolved):** Trait expanded from 29 to 32 methods: added `get_score_data_mut`, `get_course_replay_mut`, `get_course_gauge_mut`. `NullPlayerResource` changed from unit struct to `#[derive(Default)]` with `course_replay: Vec<ReplayData>` and `course_gauge: Vec<Vec<Vec<f32>>>` fields (needed for `&mut` return from null impl). Remaining gap: methods returning types from bms-model/beatoraja-core (`get_bms_model`, `get_play_mode`, `get_ranking_data`, `get_original_mode`) can't be on trait due to circular deps. beatoraja-result stub still uses incompatible types (`FloatArray` vs `Vec<f32>`, `GdxArray` vs `Vec`, `GrooveGaugeStub` vs `GrooveGauge`). Solution for non-trait types: wrapper struct pattern (like beatoraja-external).
