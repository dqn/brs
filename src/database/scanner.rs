...
impl SongScanner {
    ...
    fn parse_bms_header(
        ...
        let mode = detect_mode(&bms, path);
        let notes = count_notes(&bms, mode);
...
fn count_notes(bms: &Bms, mode: Mode) -> usize {
    use bms_rs::bms::command::channel::NoteKind;
    use bms_rs::bms::command::channel::mapper::{KeyLayoutBeat, KeyLayoutPms, KeyMapping};
    ...
    bms.notes()
        ...
        .filter(|note| {
            ...
            let (kind, side, key) = if matches!(mode, Mode::PopN5K | Mode::PopN9K) {
                let Some(layout) = KeyLayoutPms::from_channel_id(note.channel_id) else {
                    return false;
                };
                (layout.kind(), layout.side(), layout.key())
            } else {
            ...
            if !matches!(kind, NoteKind::Visible | NoteKind::Long) {
                return false;
            }

            match mode {
                Mode::PopN5K => matches!(key, Key::Key(1..=5)),
                Mode::PopN9K => matches!(key, Key::Key(1..=9)),
                Mode::Beat5K => {
                    matches!(side, PlayerSide::Player1)
                        && matches!(key, Key::Scratch(_) | Key::Key(1..=5))
                }
                Mode::Beat7K => {
                    matches!(side, PlayerSide::Player1)
                        && matches!(key, Key::Scratch(_) | Key::Key(1..=7))
                }
                Mode::Beat10K => matches!(key, Key::Scratch(_) | Key::Key(1..=5)),
                Mode::Beat14K => matches!(key, Key::Scratch(_) | Key::Key(1..=7)),
            }
        })
        .count()
}
...
fn detect_mode(bms: &Bms, source_path: &Path) -> Mode {
    use bms_rs::bms::command::channel::mapper::{KeyLayoutBeat, KeyLayoutPms, KeyMapping};
    use bms_rs::bms::prelude::{Key, PlayerSide};
    ...
    for note in bms.notes().all_notes() {
        if let Some(layout) = KeyLayoutBeat::from_channel_id(note.channel_id) {
            let key = layout.key();
            if layout.side() == PlayerSide::Player2 {
                has_2p = true;
            }
            match key {
