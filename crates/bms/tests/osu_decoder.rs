...
fn collect_lane_notes(model: &bms::model::bms_model::BMSModel) -> Vec<(i32, &Note)> {
...
fn decode_7k_fixture_metadata() {
    ...
    let mut decoder = OSUDecoder::new(bms::model::bms_model::LnType::LongNote);
    let model = decoder
        .decode_path(&path)
...
fn decode_via_chart_information() {
    ...
    let path = fixture_path("osu_7k_basic.osu");
    let info = ChartInformation::new(Some(path), bms::model::bms_model::LnType::LongNote, None);
    let mut decoder = OSUDecoder::new(bms::model::bms_model::LnType::LongNote);
    let model = decoder
        .decode(info)
