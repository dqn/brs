...
mod tests {
    ...
    fn test_difficulty_table_to_table_data_basic() {
        ...
        assert_eq!(td.get_folder()[0].get_song()[0].file.md5, "hash_a");

        // Level "2" folder
        assert_eq!(td.get_folder()[1].name(), "N2");
        assert_eq!(td.get_folder()[1].get_song().len(), 1);
        assert_eq!(td.get_folder()[1].get_song()[0].file.md5, "hash_b");
    }
    ...
    fn test_difficulty_table_to_table_data_with_mode() {
        ...
        assert_eq!(td.get_folder()[0].get_song()[0].chart.mode, 14);
    }
