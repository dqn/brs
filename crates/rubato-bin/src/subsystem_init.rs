...
use rubato::ir_resend_service::IrResendService;
...
pub(crate) fn import_lr2_scores(config: &rubato::core::config::Config) {
    let lr2_path = match rubato::platform::show_file_chooser("Select LR2 score database") {
        Some(p) => p,
        None => {
...
pub(crate) fn init_state_listeners(controller: &mut MainController) -> Vec<Box<dyn std::any::Any>> {
    ...
    if use_discord_rpc {
        let (sender, listener) = rubato::external::discord_listener::DiscordListener::new();
...
pub(crate) fn init_ir_config(controller: &mut MainController) {
    ...
    rubato::ir::ir_connection_manager::register_ir_connections(vec![
        rubato::ir::ir_connection_manager::IRConnectionEntry {
            name: rubato::ir::lr2_ir_connection_adapter::LR2IR_NAME.to_string(),
            home: Some("http://www.dream-pro.info/~lavalse/LR2IR/".to_string()),
            factory: Box::new(|| {
                Box::new(rubato::ir::lr2_ir_connection_adapter::LR2IRConnectionAdapter::new())
            }),
        },
    ]);
    ...
    for ir_status in ir_statuses {
        ...
        controller
            .ir_status_mut()
            .push(rubato::core::main_controller::IRStatus {
...
fn init_ipfs_download_processor(
    ...
    config: &rubato::core::config::Config,
) {
    match rubato::song::sqlite_song_database_accessor::SQLiteSongDatabaseAccessor::new(
        &config.paths.songpath,
        &config.paths.bmsroot,
        ...
        Ok(songdb) => {
            ...
            let processor =
                rubato::song::md_processor::music_download_processor::MusicDownloadProcessor::new(
...
fn init_http_download_processor(
    ...
    let source_meta = rubato::song::md_processor::http_download_processor::DOWNLOAD_SOURCES
        .get(&config.network.download_source)
        .copied()
        .unwrap_or_else(|| {
            rubato::song::md_processor::http_download_processor::HttpDownloadProcessor::default_download_source()
        });
    let http_download_source: Arc<
        dyn rubato::song::md_processor::http_download_source::HttpDownloadSource,
    ...
    match rubato::song::sqlite_song_database_accessor::SQLiteSongDatabaseAccessor::new(
        ...
        Ok(songdb) => {
            ...
            let info_db: Option<Box<dyn rubato::song_information_db::SongInformationDb>> =
                rubato::song::song_information_accessor::SongInformationAccessor::new(
            ...
            let processor = Arc::new(
                rubato::song::md_processor::http_download_processor::HttpDownloadProcessor::new(
                    main_ref,
...
pub(crate) fn init_stream_controller(controller: &mut MainController) {
    ...
    {
        rubato::state_factory::wire_selector_dependencies(&mut selector, controller);
