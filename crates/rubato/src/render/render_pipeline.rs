...
use crate::render::shader::{BILINEAR_SHADER_WGSL, SPRITE_SHADER_WGSL};
use crate::render::sprite_batch::SpriteVertex;
...
mod tests {
    ...
    async fn test_render_pipeline_creation() {
        let ctx = match crate::render::gpu_context::GpuContext::new_headless().await {
            Ok(ctx) => ctx,
            Err(_) => {
