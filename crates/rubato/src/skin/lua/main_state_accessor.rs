...
use crate::skin::core::skin_property_mapper;
use crate::skin::property::boolean_property_factory;
use crate::skin::property::float_property::FloatProperty;
    ...
    pub fn export(&self, lua: &Lua, table: &LuaTable) {
        let result: Result<(), LuaError> = (|| {
            ...
            let timer_func = lua.create_function(move |_, id: i32| {
                let state = unsafe { &*sp.0 };
                Ok(state.micro_timer(crate::skin::timer_id::TimerId::new(id)))
            })?;
            ...
            let set_timer_func =
                lua.create_function(move |_, (timer_id, timer_value): (i32, i64)| {
                    if !skin_property_mapper::is_timer_writable_by_skin(
                        crate::skin::timer_id::TimerId::new(timer_id),
                    ) {
...
impl SnapshotAccessor {
    ...
    pub fn export(&self, lua: &Lua, table: &LuaTable) {
        let result: Result<(), LuaError> = (|| {
            ...
            let timer_func = lua.create_function(move |_, id: i32| {
                let snapshot = unsafe { &*sp.0 };
                Ok(snapshot.micro_timer(crate::skin::timer_id::TimerId::new(id)))
            })?;
            table.set("timer", timer_func)?;
...
mod tests {
    use super::*;
    use crate::skin::reexports::{SkinOffset, Timer};
    ...
    struct LuaTestState {
        ...
        score_data_property: crate::skin::score_data_property::ScoreDataProperty,
    ...
    impl Default for LuaTestState {
        fn default() -> Self {
            Self {
                timer: Timer::default(),
                offsets: HashMap::new(),
                score_data_property: crate::skin::score_data_property::ScoreDataProperty::default(),
    ...
    impl crate::skin::timer_access::TimerAccess for LuaTestState {
    ...
    impl crate::skin::skin_render_context::SkinRenderContext for LuaTestState {
    ...
    fn test_volume_sys_returns_system_volume() {
        let mut state = LuaTestState::default();
        state.config.audio = Some(crate::skin::audio_config::AudioConfig {
            systemvolume: 0.8,
            ..Default::default()
        });
        let (_lua, table) = setup_lua_with_state(&mut state);
    ...
    fn test_set_volume_sys_calls_notify_audio_config_changed() {
        ...
        let mut state = LuaTestState::default();
        state.config.audio = Some(crate::skin::audio_config::AudioConfig::default());
        let (_lua, table) = setup_lua_with_state(&mut state);
    ...
    fn snapshot_timer_reads_from_timers_map() {
        ...
        snapshot
            .timers
            .insert(crate::skin::timer_id::TimerId::new(5), 1_000_000);
        let (_lua, table) = setup_lua_with_snapshot(&mut snapshot);
    ...
    fn snapshot_volume_sys_reads_from_config() {
        ...
        config.audio = Some(crate::skin::audio_config::AudioConfig {
            systemvolume: 0.8,
            ..Default::default()
        });
    ...
    fn snapshot_gauge_reads_from_snapshot() {
        let mut snapshot = PropertySnapshot::new();
        snapshot.state_type = Some(crate::skin::main_state_type::MainStateType::Play);
    ...
    fn snapshot_gauge_returns_zero_when_not_play_state() {
        let mut snapshot = PropertySnapshot::new();
        snapshot.state_type = Some(crate::skin::main_state_type::MainStateType::MusicSelect);
        snapshot.gauge_value = 0.85;
        let (_lua, table) = setup_lua_with_snapshot(&mut snapshot);
        let gauge_fn: mlua::Function = table.get("gauge").unwrap();
        let result: f64 = gauge_fn.call(()).unwrap();
