...
use crate::skin::lua::event_utility::EventUtility;
use crate::skin::lua::main_state_accessor::{MainStateAccessor, SnapshotAccessor};
use crate::skin::lua::timer_utility::TimerUtility;
...
impl SkinLuaAccessor {
    ...
    pub fn export_skin_property(
        &self,
        header: &crate::skin::lr2::lr2_skin_header_loader::LR2SkinHeaderData,
        ...
        let result: Result<(), LuaError> = (|| {
            ...
            let get_path_fn = self.lua.create_function(move |_, path: String| {
                let result = crate::skin::skin_loader::path(&path, &filemap);
    ...
    pub fn export_skin_property_from_header_data(
        &self,
        header: &crate::skin::json::json_skin_loader::SkinHeaderData,
        ...
        let result: Result<(), LuaError> = (|| {
            ...
            let get_path_fn = self.lua.create_function(move |_, path: String| {
                let result = crate::skin::skin_loader::path(&path, &filemap_clone);
                Ok(result.to_string_lossy().to_string())
            })?;
            table.set("path", get_path_fn)?;
...
pub struct SkinConfigProperty {
    ...
    pub offsets: Vec<crate::skin::reexports::SkinConfigOffset>,
}
