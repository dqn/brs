...
mod tests {
    use super::*;
    use crate::skin::reexports::TextureRegion;
    ...
    fn gauge_with_valid_parts_reports_no_errors() {
        ...
        let gauge = crate::skin::objects::skin_gauge::SkinGauge::new(images, 0, 0, 10, 0, 2, 100);
    ...
    fn gauge_with_zero_parts_reports_error() {
        let images: Vec<Vec<Option<TextureRegion>>> = vec![vec![Some(TextureRegion::new()); 6]];
        let gauge = crate::skin::objects::skin_gauge::SkinGauge::new(images, 0, 0, 0, 0, 2, 100);
        let issues = gauge.check_wiring();
        assert!(
            issues.iter().any(|i| i.severity == Severity::Error),
    ...
    fn bga_without_draw_reports_warning() {
        let bga = crate::skin::objects::skin_bga_object::SkinBgaObject::new(0);
