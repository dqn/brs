use crate::skin::lr2::lr2_skin_csv_loader::LR2SkinCSVLoaderState;
use crate::skin::lr2::lr2_skin_loader;
use crate::skin::reexports::{Rectangle, Texture, TextureRegion};
        ...
        match cmd {
            ...
            "DST_LINE" => {
                ...
                if idx < self.line_images.len() && self.line_images[idx].is_some() {
                    ...
                    let offset = LR2SkinCSVLoaderState::read_offset_with_base(
                        str_parts,
                        21,
                        &[crate::skin::skin_property::OFFSET_LIFT],
                    );
            ...
            "SRC_NOWJUDGE_1P" | "SRC_NOWJUDGE_2P" | "SRC_NOWJUDGE_3P" => {
                ...
                if let Some(images) = self.csv.source_image(&values) {
                    if self.judge_objects[player].is_none() {
                        ...
                        self.judge_objects[player] =
                            Some(crate::skin::skin_judge_object::SkinJudgeObject::new(
                                player as i32,
                                shift,
                            ));
                    }
                    ...
                    .is_some_and(|j| j.inner.judge(judge_idx))
                {
                        ...
                        && let Some(img) = judge_obj.judge_image_mut(judge_idx)
                    {
                        let offset_judge = match player {
                            0 => crate::skin::skin_property::OFFSET_JUDGE_1P,
                            1 => crate::skin::skin_property::OFFSET_JUDGE_2P,
                            _ => crate::skin::skin_property::OFFSET_JUDGE_3P,
                        };
                        img.data.set_destination_with_int_timer_and_offsets(
                            ...
                            values[19],
                            values[20],
                            &[offset_judge, crate::skin::skin_property::OFFSET_LIFT],
                        );
                    }
            ...
            "SRC_NOWCOMBO_1P" | "SRC_NOWCOMBO_2P" | "SRC_NOWCOMBO_3P" => {
                ...
                if let Some(simages) = self.csv.source_image(&values) {
                    ...
                    if let Some(ref mut judge_obj) = self.judge_objects[player] {
                        let number =
                            crate::skin::objects::skin_number::SkinNumber::new_with_int_timer(
                                images_2d,
                                None,
                                values[10],
                                values[9],
                                crate::skin::objects::skin_number::NumberDisplayConfig {
                                    keta: 0,
                                    zeropadding: 0,
                                    space: 0,
                                    align: 2,
            ...
            "SRC_HIDDEN" | "SRC_LIFT" => {
                ...
                let values = lr2_skin_loader::parse_int(str_parts);
                if let Some(images) = self.csv.source_image(&values) {
                    let mut h = crate::skin::objects::skin_hidden::SkinHidden::new_with_int_timer(
            ...
            "DST_HIDDEN" => {
                if let Some(ref mut hidden) = self.hidden {
                    ...
                    let offset = LR2SkinCSVLoaderState::read_offset_with_base(
                        ...
                        &[
                            crate::skin::skin_property::OFFSET_LIFT,
                            crate::skin::skin_property::OFFSET_HIDDEN_COVER,
                        ],
                    );
