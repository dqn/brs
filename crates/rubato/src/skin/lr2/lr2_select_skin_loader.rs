use crate::skin::lr2::lr2_skin_csv_loader::{LR2SkinCSVLoaderState, LR2SkinLoaderAccess};
use crate::skin::lr2::lr2_skin_loader::{self};
use crate::skin::reexports::{Rectangle, Resolution, TextureRegion};
use crate::skin::safe_div_f32;
use crate::skin::skin_bpm_graph::SkinBPMGraph;
use crate::skin::skin_image::SkinImage;
use crate::skin::skin_note_distribution_graph::SkinNoteDistributionGraph;
use crate::skin::skin_object::DestinationParams;
...
pub struct LR2SelectSkinLoaderState {
    ...
    pub barlevel: Vec<Option<crate::skin::skin_number::SkinNumber>>,
    pub bartext: Vec<Option<crate::skin::skin_text::SkinTextEnum>>,
    ...
    pub fn process_select_command(&mut self, cmd: &str, str_parts: &[String]) {
        match cmd {
                ...
                if div_total >= 10 {
                    ...
                    if let Some(images) = images {
                        ...
                        if images.len() % 24 == 0 {
                            ...
                            let sn = crate::skin::skin_number::SkinNumber::new_with_int_timer(
                                pimages,
                                Some(mimages),
                                values[10],
                                values[9],
            ...
            "SRC_BAR_TITLE" => {
                ...
                {
                    let text = crate::skin::skin_text_image::SkinTextImage::new(source);
                    crate::skin::skin_text::SkinTextEnum::Image(text)
                } else {
                    let text = crate::skin::text::skin_text_font::SkinTextFont::new(
                        "skin/default/VL-Gothic-Regular.ttf",
                        0,
                        48,
                        2,
                    );
                    crate::skin::skin_text::SkinTextEnum::Font(text)
                };
            ...
            "SRC_TEXT" => {
                ...
                self.is_search_text =
                    values[3] == crate::skin::skin_property::STRING_SEARCHWORD && values[5] != 0;
...
impl LR2SkinLoaderAccess for LR2SelectSkinLoaderState {
    ...
    fn load_skin_data(
        &mut self,
        path: &std::path::Path,
        state: Option<&dyn crate::skin::reexports::MainState>,
    ) -> anyhow::Result<()> {
    ...
    fn assemble_objects(&mut self, skin: &mut crate::skin::skin::Skin) {
        ...
        let has_bars = self
            ...
            .any(|b| b.is_some());
        if has_bars {
            let bar_obj = crate::skin::skin_bar_object::SkinBarObject::new(self.center_bar);
            skin.add(Box::new(bar_obj));

            // Transfer bar data so MusicSelector can build SkinBar + BarRenderer
            skin.select_bar_data = Some(crate::skin::select_bar_data::SelectBarData {
...
mod tests {
    ...
    fn make_loader_with_texture() -> LR2SelectSkinLoaderState {
        use crate::skin::lr2::lr2_skin_csv_loader::ImageListEntry;
        use crate::skin::render_reexports::Texture;
    ...
    fn src_bar_level_standard_sheet_timer_cycle_order() {
        ...
        use crate::skin::sources::skin_source_image_set::SkinSourceImageSet;
    ...
    fn assemble_propagates_search_text_region() {
        ...
        let mut header = crate::skin::skin_header::SkinHeader::new();
        ...
        let mut skin = crate::skin::skin::Skin::new(header);
