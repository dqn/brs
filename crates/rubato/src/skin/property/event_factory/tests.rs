...
impl crate::skin::timer_access::TimerAccess for TestMainState {
...
impl crate::skin::skin_render_context::SkinRenderContext for TestMainState {
...
fn test_bga_cycle() {
    let mut state = TestMainState::new();
    state.config.render.bga = crate::skin::config::BgaMode::Off;
    let event = event_by_id(72).unwrap();
    event.exec(&mut state, 1, 0);
    assert_eq!(state.config.render.bga, crate::skin::config::BgaMode::On); // wraps from Off
}

#[test]
fn test_bgaexpand_cycle() {
    let mut state = TestMainState::new();
    state.config.render.bga_expand = crate::skin::config::BgaExpand::Full;
    let event = event_by_id(73).unwrap();
    event.exec(&mut state, 1, 0);
    assert_eq!(
        state.config.render.bga_expand,
        crate::skin::config::BgaExpand::KeepAspectRatio
    );
}
...
fn test_notes_display_timing_clamp_max() {
    let mut state = TestMainState::new();
    state.player_config.judge_settings.judgetiming = crate::skin::player_config::JUDGETIMING_MAX;
    let event = event_by_id(74).unwrap();
    event.exec(&mut state, 1, 0);
    assert_eq!(
        state.player_config.judge_settings.judgetiming,
        crate::skin::player_config::JUDGETIMING_MAX
    );
}
