...
use crate::render::pixmap_resource_pool::PixmapResourcePool;
use crate::skin::config::Config;
use crate::skin::player_config::PlayerConfig;

use crate::skin::reexports::{MainState, Texture};
...
pub fn skin_path_from_player_config(
    ...
    player_config
        ...
        .or_else(|| crate::skin::skin_config::SkinConfig::default_for_id(skin_type_id).path)
}
...
pub fn load_skin_from_config(
    ...
    let mut skin = if skin_path.ends_with(".json") {
        ...
        let mut loader =
            crate::skin::lua::lua_skin_loader::LuaSkinLoader::new_without_state(config);
        let header = loader.load_header(&path)?;
        let data = loader.load(&path, &skin_type, &property)?;
        let skin = crate::skin::skin_data_converter::convert_skin_data(
            &header,
        ...
        // LR2 CSV skin
        let dst = crate::skin::reexports::Resolution {
...
pub fn load_skin_from_path_with_state(
    ...
    let mut skin = if skin_path.ends_with(".json") {
        ...
        let header = loader.load_header(&path)?;
        let data = loader.load(&path, &skin_type, &property)?;
        crate::skin::skin_data_converter::convert_skin_data(
            &header,
            data,
            &mut loader.source_map,
            &path,
        ...
        let mut loader =
            crate::skin::lua::lua_skin_loader::LuaSkinLoader::new_with_state(state, &config);
        let header = loader.load_header(&path)?;
        let data = loader.load(&path, &skin_type, &property)?;
        crate::skin::skin_data_converter::convert_skin_data(
            &header,
...
pub fn load_with_config(
    ...
    if skin_config_path.ends_with(".json") {
        ...
        let config = _state
            ...
            .expect("config required for skin loading");
        let mut loader =
            crate::skin::lua::lua_skin_loader::LuaSkinLoader::new_with_state(_state, &config);
        let result = loader.load_skin(Path::new(skin_config_path), skin_type, &property);
        {
            let guard = lock_or_recover(&RESOURCE);
            if let Some(ref r) = *guard {
...
mod tests {
    ...
    fn make_skin_with_offset(id: i32, name: &str) -> Skin {
        use crate::skin::skin_config_offset::SkinConfigOffset;
    ...
    fn apply_player_config_offsets_copies_values_by_name() {
        use crate::skin::skin_config::{SkinConfig, SkinOffset, SkinProperty};

        let mut skin =
            make_skin_with_offset(crate::skin::skin_property::OFFSET_ALL, "All offset(%)");
        ...
        let cfg = skin
            .offset()
            .get(&crate::skin::skin_property::OFFSET_ALL)
            .unwrap();
        assert_eq!(cfg.x, 5.0);
