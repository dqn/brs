...
use crate::skin::json::json_skin_object_loader::{source_image, utilities::resolve_wildcard_path};
...
pub(super) fn get_texture_for_src(
...
) -> Option<crate::skin::reexports::Texture> {
    let src_id = src_id?;
    ...
    let result = resolved_path
        .map(|path| SourceDataType::Texture(crate::skin::reexports::Texture::new(&path)));
