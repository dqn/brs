...
use crate::skin::reexports::{Resolution, SkinConfigOffset};
use crate::skin::types::skin::Skin;
use crate::skin::types::skin_header::{
...
pub fn convert_skin_data(
    ...
    for mut obj_data in data.objects {
        let scale_x = crate::skin::safe_div_f32(dstr.width, src.width);
        let scale_y = crate::skin::safe_div_f32(dstr.height, src.height);
        ...
        if let SkinObjectType::SearchTextRegion { x, y, w, h } = &obj_data.object_type {
            skin.search_text_region = Some(crate::skin::reexports::Rectangle::new(
                x * scale_x,
                y * scale_y,
                w * scale_x,
                h * scale_y,
        ...
        let skin_obj = if let Some(resolved) = obj_data.resolved_note.take() {
            Some(Box::new(resolved) as Box<dyn crate::skin::types::skin_node::SkinNode>)
