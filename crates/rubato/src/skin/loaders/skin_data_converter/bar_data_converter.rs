...
) -> crate::skin::select_bar_data::SelectBarData {
    crate::skin::select_bar_data::SelectBarData {
...
fn convert_bar_sub_text(
    ...
    scale_y: f32,
    filemap: &HashMap<String, String>,
) -> Vec<Option<crate::skin::skin_text::SkinTextEnum>> {
    objs.iter()
        .map(|opt_obj| {
            let obj_data = opt_obj.as_ref()?;
            let skin_obj = convert_skin_object(
            ...
            match any_obj.downcast::<SkinTextFont>() {
                Ok(mut stf) => {
                    ...
                    Some(crate::skin::skin_text::SkinTextEnum::Font(*stf))
                }
                Err(any_obj) => match any_obj.downcast::<SkinTextBitmap>() {
                    Ok(mut stb) => {
                        ...
                        Some(crate::skin::skin_text::SkinTextEnum::Bitmap(*stb))
                    }
                    Err(_) => None,
                },
            }
...
fn apply_scaled_destinations(
    ...
    destinations: &[crate::skin::json::json_skin_loader::DestinationData],
    scale_x: f32,
    scale_y: f32,
) {
...
fn apply_scaled_destinations_with_offsets(
    ...
    for dst in destinations {
        ...
        let params = crate::skin::skin_object::DestinationParams {
        ...
        if let Some(draw_id) = dst.draw
            ...
            && draw_id != -1
            && let Some(draw_prop) =
                crate::skin::property::boolean_property_factory::boolean_property(draw_id)
        {
...
mod tests {
    use super::*;
    use crate::skin::json::json_skin_loader::DestinationData;
