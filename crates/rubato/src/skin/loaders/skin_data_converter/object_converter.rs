...
use crate::skin::graphs::skin_bpm_graph::SkinBPMGraph;
use crate::skin::graphs::skin_graph::SkinGraph;
use crate::skin::graphs::skin_hit_error_visualizer::SkinHitErrorVisualizer;
use crate::skin::graphs::skin_note_distribution_graph::SkinNoteDistributionGraph;
use crate::skin::graphs::skin_timing_distribution_graph::SkinTimingDistributionGraph;
use crate::skin::graphs::skin_timing_visualizer::SkinTimingVisualizer;
use crate::skin::json::json_skin_loader::{
    SkinNumberOffset, SkinObjectType, SourceData, get_path_with_filemap,
};
use crate::skin::json::json_skin_object_loader::source_image;
use crate::skin::objects::skin_bga_object::SkinBgaObject;
use crate::skin::objects::skin_gauge::SkinGauge;
use crate::skin::objects::skin_gauge_graph_object::SkinGaugeGraphObject;
use crate::skin::objects::skin_hidden::SkinHidden;
use crate::skin::objects::skin_image::SkinImage;
use crate::skin::objects::skin_judge_object::SkinJudgeObject;
use crate::skin::objects::skin_note_object::SkinNoteObject;
use crate::skin::objects::skin_number::{NumberDisplayConfig, SkinNumber};
use crate::skin::objects::skin_slider::SkinSlider;
use crate::skin::property::string_property_factory;
use crate::skin::reexports::{SkinOffset, TextureRegion};
use crate::skin::text::skin_text_bitmap::{SkinTextBitmap, SkinTextBitmapSource};
use crate::skin::text::skin_text_font::SkinTextFont;
use crate::skin::types::skin_bar_object::SkinBarObject;
...
fn convert_image(
    ...
    if is_movie {
        ...
        let movie_source = crate::skin::skin_source_movie::SkinSourceMovie::new(&movie_path);
...
fn convert_float(
    ...
    let mut sf = if images.len().is_multiple_of(26) {
        ...
        crate::skin::skin_float::SkinFloat::new_with_int_timer_int_id_mimage(
            pn,
            Some(mn),
            timer_val,
            cycle,
        ...
        crate::skin::skin_float::SkinFloat::new_with_int_timer_int_id(
            nimage,
            timer_val,
            cycle,
            crate::skin::skin_float::FloatDisplayConfig {
    ...
    if let Some(font_path) = font {
        ...
        if is_bitmap_font {
            ...
            if !outline_color.is_empty() && outline_color != "ffffff00" {
                stb.text_data
                    .set_outline_color(crate::skin::reexports::Color::value_of(outline_color));
            }
            stb.text_data.outline_width = outline_width;
            if !shadow_color.is_empty() && shadow_color != "ffffff00" {
                stb.text_data
                    .set_shadow_color(crate::skin::reexports::Color::value_of(shadow_color));
            }
            stb.text_data
...
mod tests {
    ...
    use crate::skin::text::skin_text_bitmap::SkinTextBitmap;
    use crate::skin::text::skin_text_font::SkinTextFont;
    use crate::skin::types::skin_node::SkinNode;
    ...
    fn convert_image_creates_object_with_empty_refs_when_srcimg_fewer_than_len() {
        ...
        use crate::skin::json::json_skin_loader::{SourceData, SourceDataType};
        ...
        assert!(
            result
                .unwrap()
                .as_any()
                .downcast_ref::<crate::skin::objects::skin_image::SkinImage>()
                .is_some(),
    ...
    fn make_source_map_with_image_count(
        ...
        std::collections::HashMap<String, crate::skin::json::json_skin_loader::SourceData>,
        i32,
        i32,
    ) {
        use crate::skin::json::json_skin_loader::{SourceData, SourceDataType};
    ...
    fn convert_float_26_layout_preserves_sign_visible() {
        ...
        let sf = obj
            .as_any()
            .downcast_ref::<crate::skin::core::skin_float::SkinFloat>()
    ...
    fn convert_slider_scales_range_by_direction() {
        ...
        let sl = obj
            .as_any()
            .downcast_ref::<crate::skin::objects::skin_slider::SkinSlider>()
...
fn convert_slider(
    ...
    let slider = if let Some(val_id) = value {
        ...
        SkinSlider::new_with_int_timer_minmax(
            crate::skin::objects::skin_slider::SliderIntTimerMinmaxParams {
                image: images,
                timer: timer_val,
                cycle,
                angle,
...
fn convert_gauge_graph(
    ...
    let gg = if let Some(colors) = color {
        ...
        SkinGaugeGraphObject::new_from_color_strings(
            &crate::skin::objects::skin_gauge_graph_object::GaugeGraphColorStrings {
...
fn convert_bpm_graph(
    ...
    let graph = SkinBPMGraph::new(crate::skin::skin_bpm_graph::BpmGraphConfig {
        delay,
        line_width,
        main_bpm_color,
        min_bpm_color,
...
fn convert_hit_error_visualizer(
    ...
    let viz = SkinHitErrorVisualizer::new(
        crate::skin::skin_hit_error_visualizer::HitErrorVisualizerConfig {
            width,
            judge_width_millis,
            line_width,
            color_mode,
...
fn convert_timing_visualizer(
    ...
    let viz = SkinTimingVisualizer::new(
        crate::skin::skin_timing_visualizer::TimingVisualizerConfig {
            width,
            judge_width_millis,
            line_width,
            line_color,
...
fn convert_timing_distribution_graph(
    ...
    let graph = SkinTimingDistributionGraph::new(
        crate::skin::skin_timing_distribution_graph::TimingDistributionGraphConfig {
            width,
            line_width,
            graph_color,
            average_color,
