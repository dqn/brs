...
use crate::skin::json::json_skin_object_loader::{self, JsonSkinObjectLoader};
use crate::skin::property::boolean_property_factory;
use crate::skin::property::timer_property_factory;
use crate::skin::reexports::Resolution;
use crate::skin::types::skin_object::DestinationParams;
...
fn apply_runtime_skin_object_data(
    obj: &mut dyn crate::skin::types::skin_node::SkinNode,
    obj_data: &SkinObjectData,
    src: &Resolution,
    dst: &Resolution,
) {
    let dw = crate::skin::safe_div_f32(dst.width, src.width);
    let dh = crate::skin::safe_div_f32(dst.height, src.height);
...
fn prime_named_object_texture(
    ...
    for img in &sk.image {
        if img.id.as_deref() == Some(dst_id) {
            let _ = crate::skin::json::json_skin_object_loader::utilities::texture(
                loader,
                img.src.as_deref(),
                p,
            );
...
fn resolve_judge_child_skin_object(
    ...
    p: &Path,
) -> Option<Box<dyn crate::skin::types::skin_node::SkinNode>> {
    ...
    let scale_x = crate::skin::safe_div_f32(loader.dstr.width, src.width);
    let scale_y = crate::skin::safe_div_f32(loader.dstr.height, src.height);
    let mut obj = crate::skin::skin_data_converter::convert_runtime_object(
...
fn build_resolved_judge(
    ...
    p: &Path,
) -> crate::skin::skin_judge_object::SkinJudgeObject {
    let mut judge_obj =
        crate::skin::skin_judge_object::SkinJudgeObject::new(judge.index, judge.shift);
    ...
    for (idx, image_dst) in judge.images.iter().enumerate().take(7) {
        match resolve_judge_child_skin_object(loader, skin, sk, image_dst, p) {
            Some(obj) => {
                match obj
                    .into_any_box()
                    .downcast::<crate::skin::skin_image::SkinImage>()
                {
                    Ok(image) => {
    ...
    for (idx, number_dst) in judge.numbers.iter().enumerate().take(7) {
        match resolve_judge_child_skin_object(loader, skin, sk, number_dst, p) {
            Some(obj) => {
                match obj
                    .into_any_box()
                    .downcast::<crate::skin::skin_number::SkinNumber>()
                {
                    Ok(number) => {
...
impl JsonSkinObjectLoader for JsonPlaySkinObjectLoader {
    fn skin(&self, header: &crate::skin::json::json_skin_loader::SkinHeaderData) -> SkinData {
        ...
        let skin_type = crate::skin::skin_type::SkinType::skin_type_by_id(header.skin_type)
            .unwrap_or(crate::skin::skin_type::SkinType::Play7Keys);
    ...
    fn load_skin_object(
        ...
        {
            use crate::skin::json::json_skin_object_loader::utilities::note_texture;
            use crate::skin::skin_note_object::SkinNoteObject;
            ...
            for (i, anim) in note.dst.iter().enumerate() {
                note_obj.inner.set_lane_region(
                    i,
                    &crate::skin::skin_note::LaneRegion {
                        x: anim.x as f32,
                        y: anim.y as f32,
                        width: anim.w as f32,
...
mod tests {
    use super::*;
    use crate::skin::json::json_skin_loader::SkinHeaderData;
    use crate::skin::json::json_skin_loader::{SkinConfigProperty, parse_skin_json};
    use crate::skin::json::json_skin_object_loader::JsonSkinObjectLoader;
    use crate::skin::loaders::skin_data_converter::convert_skin_data;
    use crate::skin::reexports::Resolution;
    use crate::skin::skin_judge_object::SkinJudgeObject;
