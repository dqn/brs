...
use beatoraja_play::judge_algorithm::DEFAULT_ALGORITHM;
use beatoraja_play::target_property::TargetProperty;
use beatoraja_types::main_state_type::MainStateType;
use beatoraja_types::play_config;
...
impl Event for SortEvent {
    fn exec(&self, state: &mut dyn MainState, arg1: i32, _arg2: i32) {
        ...
        let len = beatoraja_types::bar_sorter::BarSorter::DEFAULT_SORTER.len() as i32;
        let Some(config) = state.get_player_config_mut() else {
            return;
        };
        ...
        config.sort = next;
        config.sortid = Some(
            beatoraja_types::bar_sorter::BarSorter::DEFAULT_SORTER[next as usize]
                .name()
                .to_string(),
        );
        state.update_bar_after_change();
...
impl Event for SongbarSortEvent {
    fn exec(&self, state: &mut dyn MainState, arg1: i32, _arg2: i32) {
        ...
        let all = &beatoraja_types::bar_sorter::BarSorter::ALL_SORTER;
        let len = all.len();
        let Some(config) = state.get_player_config_mut() else {
            return;
        };
...
struct PlayerConfigCycleEvent {
    event_id: i32,
    get: fn(&beatoraja_types::player_config::PlayerConfig) -> i32,
    set: fn(&mut beatoraja_types::player_config::PlayerConfig, i32),
    count: i32,
...
struct ConfigCycleEvent {
    event_id: i32,
    get: fn(&beatoraja_types::config::Config) -> i32,
    set: fn(&mut beatoraja_types::config::Config, i32),
    count: i32,
}
...
impl Event for NotesDisplayTimingEvent {
    fn exec(&self, state: &mut dyn MainState, arg1: i32, _arg2: i32) {
        ...
        let max = beatoraja_types::player_config::JUDGETIMING_MAX;
        let min = beatoraja_types::player_config::JUDGETIMING_MIN;
        let inc = if arg1 >= 0 {
...
mod tests {
    ...
    struct TestMainState {
        ...
        player_config: beatoraja_types::player_config::PlayerConfig,
        config: beatoraja_types::config::Config,
        play_config: beatoraja_types::play_config::PlayConfig,
        is_selector: bool,
    ...
    impl TestMainState {
        fn new() -> Self {
            Self {
                ...
                resource: PlayerResource,
                player_config: beatoraja_types::player_config::PlayerConfig::default(),
                config: beatoraja_types::config::Config::default(),
                play_config: beatoraja_types::play_config::PlayConfig::default(),
                is_selector: true,
    ...
    impl MainState for TestMainState {
        ...
        fn get_selected_play_config_ref(
            &self,
        ) -> Option<&beatoraja_types::play_config::PlayConfig> {
    ...
    fn test_notes_display_timing_clamp_max() {
        let mut state = TestMainState::new();
        state.player_config.judgetiming = beatoraja_types::player_config::JUDGETIMING_MAX;
        let event = get_event_by_id(74).unwrap();
        event.exec(&mut state, 1, 0);
        assert_eq!(
            state.player_config.judgetiming,
            beatoraja_types::player_config::JUDGETIMING_MAX
        );
    }
