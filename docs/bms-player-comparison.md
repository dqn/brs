...
LR2、beatoraja、bms-rs の 3 プレイヤーを比較。

## 概要

| 項目 | LR2 | beatoraja | bms-rs |
|------|-----|-----------|--------|
| 開発者 | Lavalse 他 | exch | - |
| 言語 | C++ | Java | Rust |
...
| 判定 | LR2 | beatoraja (7keys) | bms-rs (beatoraja) | bms-rs (LR2) |
|------|-----|-------------------|-------------------|--------------|
...
bms-rs は `JudgeSystemType` で beatoraja/LR2 の判定を切り替え可能。非対称 BAD 判定にも対応。

### 判定難易度による倍率

beatoraja/bms-rs は EASY 判定に対して倍率を適用:
...
| bms-rs | LR2 モード時: ノート数ベースのダメージ倍率（1000未満で増加） |
...
| bms-rs | ○ | 全ゲージを並列追跡、失敗時に次ゲージへ自動シフト |
...
bms-rs は ffmpeg-next を使用し、ffmpeg 対応フォーマットをサポート。

## プレイオプション

### レーンオプション

| オプション | LR2 | beatoraja | bms-rs | 説明 |
|------------|-----|-----------|--------|------|
| MIRROR | ○ | ○ | ○ | レーンを左右反転 |
| RANDOM | ○ | ○ | ○ | レーン位置をランダム化 |
