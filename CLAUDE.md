...
- **Table/Course システム** — ~~~70% 完了~~ ~~**~95% 完了**~~ **完了** — jbmstable-parser Rust 移植 (`bms-database/src/difficulty_table_parser.rs`)、MusicSelect テーブルフォルダ表示 (`Bar::TableRoot`/`Bar::HashFolder` + `load_tables()` + `enter_folder()` 拡張)、HTTP テーブルダウンロード/更新 (`brs/src/table_updater.rs` + バックグラウンドフェッチ)、`data_rule` レベルリマッピング (`apply_data_rule()` + 複数 `data_url` マージ時の適用)
