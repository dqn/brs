...
//
// Skin coordinate system (matching LibGDX / beatoraja):
...
/// Convert skin coordinates (bottom-left origin, Y-up) to Bevy position (center origin, Y-up).
/// Skin objects are positioned by their bottom-left corner; Bevy sprites use center anchor.
pub fn skin_to_bevy_position(x: f32, y: f32, w: f32, h: f32, screen_w: f32, screen_h: f32) -> Vec2 {
    let bevy_x = (x + w * 0.5) - screen_w * 0.5;
    let bevy_y = (y + h * 0.5) - screen_h * 0.5;
    Vec2::new(bevy_x, bevy_y)
}

/// Convert skin coordinates to a full Bevy Transform with z-order and rotation.
/// `angle_deg` is rotation in degrees (counter-clockwise, matching LibGDX convention).
/// `center_x`/`center_y` are 0.0–1.0 relative to the object size (0.5, 0.5 = center).
/// `center_y` follows LibGDX convention: 0.0 = bottom, 1.0 = top.
pub fn skin_to_bevy_transform(
    ...
    // Position: convert skin bottom-left to Bevy center
    let pos = skin_to_bevy_position(x, y, w, h, screen_w, screen_h);

    // Rotation: both skin (LibGDX) and Bevy use counter-clockwise convention
...
mod tests {
    ...
    fn test_bottom_left_corner() {
        // Skin (0,0) with 100x100 on 1280x720 — bottom-left of screen
        // bevy_x = (0 + 50) - 640 = -590
        // bevy_y = (0 + 50) - 360 = -310
    ...
    fn test_top_right_corner() {
        // Top-right: x=1180, y=620, w=100, h=100 on 1280x720
        // bevy_x = (1180 + 50) - 640 = 590
        // bevy_y = (620 + 50) - 360 = 310
    ...
    fn test_transform_90_degree_center_rotation() {
        // 90 degree counter-clockwise rotation around center (0.5, 0.5)
        let t = skin_to_bevy_transform(
            SkinRect {
                x: 100.0,
                y: 100.0,
    ...
    fn test_transform_rotation_non_center_point() {
        // 90 degree CCW rotation around bottom-left corner (center_x=0, center_y=0)
        let t = skin_to_bevy_transform(
            SkinRect {
                x: 200.0,
                y: 200.0,
        ...
        // After 90 degree CCW (+PI/2), rotated offset: (50, -50)
    ...
    #[test]
    fn test_symmetry_bottom_left_top_right() {
        // Bottom-left and top-right of same-size objects should be symmetric about origin
        let bl = skin_to_bevy_position(0.0, 0.0, 100.0, 100.0, SCREEN_W, SCREEN_H);
        let tr = skin_to_bevy_position(1180.0, 620.0, 100.0, 100.0, SCREEN_W, SCREEN_H);
        assert!(approx_eq_f32(bl.x, -tr.x, 0.01));
        assert!(approx_eq_f32(bl.y, -tr.y, 0.01));
    }
