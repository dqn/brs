...
impl JudgeManager {
    ...
    pub fn new(config: &JudgeConfig<'_>) -> Self {
        ...
        // BSS tracking
        let scratch_count = play_mode.scratch_keys().len();
        let sckey = vec![0i32; scratch_count];

        // Autoplay press times
        let auto_presstime = vec![NOT_SET; key_count];
    ...
    /// * `key_states` - Per-lane key state (true = pressed), length = key_count
    /// * `key_changed_times` - Per-lane last key change time (NOT_SET if no change), length = key_count
    ...
    fn phase_pass(
        ...
        for lane_idx in 0..lane_count {
            ...
            let pressed = key_states.get(lane_idx).copied().unwrap_or(false);
            ...
            for note_idx in note_indices {
                ...
                if self.autoplay && self.note_states[note_idx] == 0 {
                    match note.note_type {
                        NoteType::Normal => {
                            self.auto_presstime[lane_idx] = time_us;
                            if note.wav_id > 0 {
                                events.push(JudgeEvent::KeySound {
                                    wav_id: note.wav_id,
                                });
            ...
            // Autoplay: release keys after min duration
            if self.autoplay
                && self.lane_states[lane_idx].processing == NO_NOTE
                && self.auto_presstime[lane_idx] != NOT_SET
                && time_us - self.auto_presstime[lane_idx] > AUTO_MIN_DURATION
            {
                self.auto_presstime[lane_idx] = NOT_SET;
            }
    ...
    fn phase_key_input(
        ...
        for lane_idx in 0..lane_count {
            let pmtime = key_changed_times[lane_idx];
            if pmtime == NOT_SET {
                continue;
            }
            let pressed = key_states[lane_idx];
            ...
            if pressed {
                ...
                if self.lane_states[lane_idx].processing != NO_NOTE {
                    ...
                    if is_cn_hcn {
                        ...
                        // LN release: worst of (start, end)
                        let mut final_judge = judge.max(self.lane_states[lane_idx].ln_start_judge);
                        let final_dmtime =
                            if self.lane_states[lane_idx].ln_start_duration.abs() > dmtime.abs() {
...
mod tests {
    ...
    fn mine_triggers_damage_when_pressed() {
        ...
        let key_states = vec![true, false, false, false, false, false, false, false];
        let key_times = vec![NOT_SET; 8]; // No key change, just held
