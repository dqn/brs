// Property ID types and constants ported from SkinProperty.java.
//
// All property values are stored as plain i32 IDs. Evaluation logic
// (resolving IDs to actual values) is deferred to Phase 11 (State Machine).
...
/// Integer property ID.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub struct IntegerId(pub i32);

/// Boolean property ID. Negative values mean "negated" (i.e. NOT condition).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub struct BooleanId(pub i32);

/// Float property ID.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub struct FloatId(pub i32);
...
impl BooleanId {
    /// Returns true if this boolean ID is negated (negative value).
    pub fn is_negated(self) -> bool {
        self.0 < 0
    }

    /// Returns the absolute ID (for lookup), stripping negation.
    pub fn abs_id(self) -> i32 {
        self.0.abs()
    }
}
...
impl_display!(TimerId);
impl_display!(IntegerId);
impl_display!(BooleanId);
impl_display!(FloatId);
impl_display!(StringId);
...
pub const NUMBER_FOLDER_GROOVE: i32 = 325;
...
// ---------------------------------------------------------------------------
// RATE (float) constants — used as bar graph / slider ratios
// ---------------------------------------------------------------------------
...
pub const FLOAT_TIMING_STDDEV: i32 = 376;
...
pub const BUTTON_BOTTOMSHIFTABLEGAUGE: i32 = 341;
pub const BUTTON_RIVAL: i32 = 79;
pub const BUTTON_FAVOURITE_SONG: i32 = 89;
pub const BUTTON_FAVOURITE_CHART: i32 = 90;
...
mod tests {
    ...
    #[test]
    fn test_boolean_id_negation() {
        let positive = BooleanId(42);
        assert!(!positive.is_negated());
        assert_eq!(positive.abs_id(), 42);

        let negative = BooleanId(-42);
        assert!(negative.is_negated());
        assert_eq!(negative.abs_id(), 42);
    }

    #[test]
    fn test_timer_id_display() {
        assert_eq!(TimerId(50).to_string(), "50");
    }

    #[test]
    fn test_timer_ranges() {
        // Bomb timers: 1P keys 0-9 at 50-59, extended 10-99 at 1010-1099
        assert_eq!(TIMER_BOMB_1P_SCRATCH, 50);
        assert_eq!(TIMER_BOMB_1P_KEY1, 51);
        assert_eq!(TIMER_BOMB_1P_KEY10, 1010);
        assert_eq!(TIMER_BOMB_1P_KEY99, 1099);

        // Custom timer range
        assert!(TIMER_CUSTOM_BEGIN > TIMER_MAX);
        assert_eq!(TIMER_CUSTOM_END - TIMER_CUSTOM_BEGIN, 9999);
    }

    #[test]
    fn test_offset_range() {
        assert!(OFFSET_ALL < OFFSET_MAX);
        assert!(OFFSET_NOTES_1P < OFFSET_MAX);
    }
}
