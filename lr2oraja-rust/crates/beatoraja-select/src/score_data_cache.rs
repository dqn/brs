...
pub struct ScoreDataCache {
    ...
    read_single: Box<dyn Fn(&SongData, i32) -> Option<ScoreData> + Send + Sync>,
    /// Function to read multiple scores from the source
    read_multi: Box<dyn Fn(&dyn Fn(&SongData, Option<&ScoreData>), &[SongData], i32) + Send + Sync>,
}
...
impl ScoreDataCache {
    pub fn new(
        read_single: Box<dyn Fn(&SongData, i32) -> Option<ScoreData> + Send + Sync>,
        read_multi: Box<
            dyn Fn(&dyn Fn(&SongData, Option<&ScoreData>), &[SongData], i32) + Send + Sync,
        >,
    ) -> Self {
        Self {
            scorecache: [
